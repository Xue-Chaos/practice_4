...
void main(void)
{
   ...
   while(1)
    ...
    //=====》空白处开始
    sprintf(SprintfAsciiResult,"传感器电压值：%s",AsciiStr);
    UART0SendData(SprintfAsciiResult,sizeof(SprintfAsciiResult) );
    ...
    //《=====空白处结束
     ...
     //由考生完成向串口发送可燃气体协议帧
    //=====》空白处开始
    BuildProtocolFrame(get_adc());
    UART0SendData(ProtocolBuff,8);
